...
# 4. RUN2 Confusion matrix logistic regression results
```{r}
predicted_labels_LGR <- read.csv(file = "/media/Helios_scStorage/Mariano/NN_Human_Mice/multinomial_logistic_regression_model/new_run/one_hot_encoded_predictions_test_data_LR_multinomial.csv", header = TRUE)
predicted_labels_LGR <- data.frame(lapply(predicted_labels_LGR, as.numeric))
...
p1
ggsave("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/confusion_matrix_plots/Run2_LGR_confusion_matrix_celltype.pdf", plot=p1, width = 8, height = 8)
...
p1 <- plot_confusion_matrix(conf_mat_dis$`Confusion Matrix`[[1]],
                      ...
                      arrow_size = 0.3)
p1
ggsave("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/confusion_matrix_plots/Run2_LGR_confusion_matrix_disease.pdf", plot=p1, width = 10, height = 10)
...
p1 <- plot_confusion_matrix(conf_mat_spec$`Confusion Matrix`[[1]],
                      ...
                      arrow_size = 0.4)
p1
ggsave("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/confusion_matrix_plots/Run2_LGR_confusion_matrix_species.pdf", plot=p1, width = 10, height = 10)
...
p1 <- ggplot(comb_df, aes(x=CCR, y= percentage, fill=Model))+
  ...
  theme_classic()+
    ...
    scale_fill_manual(values = plot_colors)
p1
ggsave(plot = p1, filename = "/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/barplot_CCR/Run2_CCR_Barplot_LGR_NN.pdf", width = 6, height = 5)
```

# 5. RUN3 3 major class confusion matrix logistic regression results
```{r}
...
##Additional: Confusion matrix plot for disease mouse results
```{r}
#Now for Disease
test_labels.disease <- test_labels[,colnames(test_labels) %in% c('AS', 'HFpEF', 'HFrEF', 'CTRL')]
predicted_labels.disease <- predicted_labels[,colnames(predicted_labels) %in% c('AS', 'HFpEF', 'HFrEF', 'CTRL')]
...
#for actual
row_indices_mouse <- which(actual.df$disease_encoded == 2) 
test_labels.disease.m <- test_labels.disease[row_indices_mouse,]
#for pred
predicted_labels.disease.m <- predicted_labels.disease[row_indices_mouse,]

# Create a vector of cell types
diseases <- colnames(test_labels.disease.m) #"1 AS" "2 HFpEF"    "3 HFrEF"    "4 CTRL

# Create DataFrame with column containing cell type numbers
new_data <- apply(test_labels.disease.m, 1, function(row) {
  disease_index <- which.max(row)
  return(disease_index)
})
...
# Create DataFrame with column containing cell type numbers
new_data <- apply(predicted_labels.disease.m, 1, function(row) {
  disease_index <- which.max(row)
  return(disease_index)
})
...
ggsave("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/confusion_matrix_plots/confusion_matrix_disease_m.pdf", plot=p1, width = 10, height = 10)
dev.off()
``` 