...
test_labels <- read.csv(file = "/media/Helios_scStorage/Mariano/NN_Human_Mice/f1_loss_runs_23_10_16/test_labels_binarized_AUTO_5000_2000_500_MLP_500_200_100_Batch_10240_SEED_1234.csv", header = FALSE)
...
predicted_labels <- read.csv(file = "/media/Helios_scStorage/Mariano/NN_Human_Mice/f1_loss_runs_23_10_16/predicted_binarized_AUTO_5000_2000_500_MLP_500_200_100_Batch_10240_SEED_1234.csv", header = FALSE)
...
# 5. Confusion matrix logistic regression results
```{r}
...
NN_df <- data.frame(CCR = c("Species", "Cell type", "Disease"),
                    percentage = c(99.90, 89.42, 84.33),
                    Model = "NN")
# combine
...
NN_dot_df <- data.frame(CCR = c(rep("Species",2),
                                 rep("Cell type",7),
                                 rep("Disease",4)),
                         percentage = c(100,99.8,
                                        97.3,97.3,94,85.6,73.8, 93.1, 84.8,
                                        99.8, 73.1, 90.8, 73.6),
                         Model = "NN")
...
# ListTest <- list()
# ListTest[[1]] <- c("Species_LGR", "Species_NN")
# ListTest[[2]] <- c("Cell type_LGR", "Cell type_NN")
# ListTest[[3]] <- c("Disease_LGR", "Disease_NN")
# 
# comb_dot_df_test <- comb_dot_df
# comb_dot_df_test$CCR <- paste(comb_dot_df$CCR, comb_dot_df$Model, sep = "_")
# stat.test <- comb_dot_df_test %>%
# rstatix::wilcox_test(percentage ~ CCR, comparisons = ListTest, p.adjust.method = "none") %>%
# rstatix::add_significance()
# stat.test$p.adj <- ifelse(stat.test$p.adj == 0, sprintf("%.2e",.Machine$double.xmin), sprintf("%.2e", stat.test$p.adj))

plot_colors <- c("#1f77b4","#ff7f0e")
