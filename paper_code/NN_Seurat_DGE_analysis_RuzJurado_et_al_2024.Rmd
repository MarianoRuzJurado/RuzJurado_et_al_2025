...
#3. Volcano plot for genes found by Seurat
##3.1 For HUMAN 
```{r}
outputFolder <- "/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/volcanos/Seurat/Human"

library(EnhancedVolcano)
cellnames <- unique(Seu.Obj.Human$cell_type)
diseasenames <- c("Hypertrophy", "HFpEF", "HFrEF")
Idents(SeuratObject.combined)<-"celltypes"

for (cell_type in cellnames) {
  Human_markers_final_cell <- subset(Human_markers_final, subset = celltype == cell_type)
  
  for (dis in diseasenames) {
    
    Human_markers_final_cell_dis <- subset(Human_markers_final_cell, subset = disease == dis)
    p <- EnhancedVolcano(Human_markers_final_cell_dis,
    Human_markers_final_cell_dis$gene,
    selectLab = Human_markers_final_cell_dis[abs(Human_markers_final_cell_dis$avg_log2FC) >= 2.5,]$gene,
    x = 'avg_log2FC',
    y = 'p_val_adj',
    title = paste0(cell_type," - ", dis, " vs CTRL"),
    pCutoff = 10e-2,
    FCcutoff = 0.5,
    pointSize = 1.0,
    labSize = 2.5,
    ...
    p
    ggsave(filename = paste0(outputFolder,"/","Volcano_Human_",cell_type,"_",dis,"_CTRL.pdf"),
           plot = p, device = "pdf", height = 11, width = 10.5)
  }
