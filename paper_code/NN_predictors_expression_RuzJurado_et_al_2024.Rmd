...
#initalize cell type and disease, set testmethod to load
  ...
  for (cell_type in cell_names) {
    ...
    if (metric == "fc_p") {
      ...
      egallgenes <- egallgenes %>%
        left_join(seurat_DGE_sub %>% select(rowname, avg_log2FC, p_val_adj), by = c("SYMBOL" = "rowname"))
  
      egallgenes$p_val_adj <- ifelse(egallgenes$p_val_adj == 0, (.Machine$double.xmin*10), egallgenes$p_val_adj)
      
      egallgenes <- egallgenes %>%
       mutate(stat = sign(avg_log2FC) * log10(abs(avg_log2FC / p_val_adj)+1))
    ...
    if (plotting == TRUE) {
      ...
      ggsave(filename = paste0("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/GSEA_analysis_XAI_seurat/seurat_",method.test,"/",cell_type,"_",disease,"/ridgeplot_",cell_type,"_",disease,"_",method.test,"_seurat",metric,".pdf"), width = 7, height = 9.5)
    }
  }
}
  ...
  for (cell_type in cell_names) {
    ...
    if (metric == "fc_p") {
      ...
      egallgenes_xai <- egallgenes_xai %>%
        left_join(xai_DGE %>% select(Feature, avg_cube_root_FC, p_adj), by = c("SYMBOL" = "Feature"))
  
      egallgenes_xai$p_adj <- ifelse(egallgenes_xai$p_adj == 0, (.Machine$double.xmin*10), egallgenes_xai$p_adj)
  
      #Rank genes by combining fold change and p-values
      # egallgenes_xai <- egallgenes_xai %>%
      #   mutate(stat = sign(avg_cube_root_FC) * log10(abs(avg_cube_root_FC / p_adj)+1))
  
      #normal formula used in bulk
      # egallgenes_xai <- egallgenes_xai %>%
      #   mutate(stat = sign(avg_cube_root_FC) * log10(p.adj_bon))
    ...
    ggsave(filename = paste0("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/GSEA_analysis_XAI_seurat/XAI_wilcox/",cell_type,"_",disease,"/ridgeplot_",cell_type ,"_",disease,"_",method,"_XAI_",metric,".pdf"), width = 7, height = 9.5)
    }
  }
}  
```
...
#12. GSEA score calculation HFpEF
##12.1 For CM
```{r}
library(GO.db)
...
filtered_go_CM <- go_terms[go_terms$Term %in% go_terms$Term[grepl("heart|cardiac|muscle|myocardium|contraction|sarcomere|cardiogenesis|hypertrophy|electrophysiology|ion transport|gap junction|mitochondria|angiogenesis|apoptosis|fibrosis",go_terms$Term, ignore.case = TRUE)],]
filtered_go_CM <- filtered_go_CM[filtered_go_CM$Ontology == "BP",]
heart_cardiac_muscle_GOs <- unique(filtered_go_CM$go_id)
...
TP = nr.inGO.seu
FP = nr.total.seu - TP
FN = length(heart_cardiac_muscle_GOs) - TP

Prec.Seu  = TP / (TP+FP)
Rec.Seu = TP / (TP+FN)

F1.Seu = 2*(Prec.Seu*Rec.Seu/(Prec.Seu+Rec.Seu))
...
F1.XAI = 2*(Prec.XAI*Rec.XAI/(Prec.XAI+Rec.XAI))
...
# "angiogenesis": The formation of new blood vessels, a key function of endothelial cells.
# "vascularization": The process of forming blood vessels, relevant in development and healing.
# "inflammation": Endothelial cells play a significant role in the inflammatory response.
# "vascular permeability": Refers to the ability of substances to pass through blood vessel walls.
# "platelet activation": Endothelial cells are involved in the regulation of platelet function.
# "shear stress": Refers to the stress on endothelial cells due to blood flow, influencing their function and morphology.
# "cell adhesion": The process by which endothelial cells interact with other cells, important in immune response and inflammation.
# "smooth muscle contraction": Endothelial cells influence the contraction of underlying smooth muscle in blood vessels.
# "endothelial dysfunction": Refers to a pathological condition affecting endothelial cells, relevant in cardiovascular diseases.
# "nitric oxide production": Endothelial cells produce nitric oxide, a key molecule in vasodilation.
# "extracellular matrix": The network of proteins and molecules that provide structural support to endothelial cells.
# "hypoxia": Low oxygen conditions that can influence endothelial cell behavior and angiogenesis.
# "vascular remodeling": Changes in the structure of blood vessels, involving endothelial cells.

filtered_go_EC <- go_terms[go_terms$Term %in% go_terms$Term[grepl("endothelium|endothelial|angiogenesis|vascularization|inflammation|vascular permeability|platelet activation|shear stress|cell adhesion|smooth muscle contraction|endothelial dysfunction|nitric oxide production|extracellular matrix|hypoxia|vascular remodeling",go_terms$Term, ignore.case = TRUE)],]

filtered_go_EC <- filtered_go_EC[filtered_go_EC$Ontology == "BP",]
heart_endothelial_GOs <- unique(filtered_go_EC$go_id)

GO.GSEA_seu <- read.xlsx("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/excelsheets/GSEAs/GSEA_Endothelial_HFpEF_wilcox_seurat.xlsx", rowNames=T)
#Only significant
GO.GSEA_seu <- GO.GSEA_seu[GO.GSEA_seu$p.adjust <= 0.05,]


nr.inGO.seu = length(GO.GSEA_seu$ID[GO.GSEA_seu$ID %in% heart_endothelial_GOs])
nr.total.seu = length(GO.GSEA_seu$ID)
ratio.seu = nr.inGO.seu/nr.total.seu
...
GO.GSEA_xai <- read.xlsx("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/excelsheets/GSEAs/GSEA_Endothelial_HFpEF_t_xai.xlsx", rowNames=T)
#Only significant
GO.GSEA_xai <- GO.GSEA_xai[GO.GSEA_xai$p.adjust <= 0.05,]

nr.inGO.xai = length(GO.GSEA_xai$ID[GO.GSEA_xai$ID %in% heart_endothelial_GOs])
nr.total.xai = length(GO.GSEA_xai$ID)
ratio.xai = nr.inGO.xai/nr.total.xai
```
...
cell_names <- c("Cardiomyocytes","Endothelial","Fibroblasts","Immune.cells","Neuro","Pericytes","Smooth.Muscle")
path_GSEA <- "/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/excelsheets/GSEAs/"
...
for (celltype in cell_names) {
  ...
  #Seurat ratio
  GO.GSEA_seu <- read.xlsx(paste0(path_GSEA,"GSEA_",celltype,"_HFpEF_wilcox_seurat.xlsx"), rowNames=T)
  GO.GSEA_seu <- GO.GSEA_seu[GO.GSEA_seu$p.adjust <= 0.05,] #Only significant
  
  nr.inGO.seu = length(GO.GSEA_seu$ID[GO.GSEA_seu$ID %in% go_id])
  nr.total.seu = length(GO.GSEA_seu$ID)
  ratio.seu = nr.inGO.seu/nr.total.seu
  
  #Precision and Recall Seurat
  TP.Seu = nr.inGO.seu
  FP.Seu = nr.total.seu - TP.Seu
  FN.Seu = length(go_id) - TP.Seu
  
  Prec.Seu  = TP.Seu / (TP.Seu+FP.Seu)
  Rec.Seu = TP.Seu / (TP.Seu+FN.Seu)
  
  F1.Seu = 2*(Prec.Seu*Rec.Seu/(Prec.Seu+Rec.Seu))
  
  
  #MAST ratio
  GO.GSEA_MAST <- read.xlsx(paste0(path_GSEA,"GSEA_",celltype,"_HFpEF_MAST_seurat.xlsx"), rowNames=T)
  GO.GSEA_MAST <- GO.GSEA_MAST[GO.GSEA_MAST$p.adjust <= 0.05,] #Only significant
  
  nr.inGO.MAST = length(GO.GSEA_MAST$ID[GO.GSEA_MAST$ID %in% go_id])
  nr.total.MAST = length(GO.GSEA_MAST$ID)
  ratio.MAST = nr.inGO.MAST/nr.total.MAST
  
  #Precision and Recall MAST
  TP.MAST = nr.inGO.MAST
  FP.MAST = nr.total.MAST - TP.MAST
  FN.MAST = length(go_id) - TP.MAST
  
  Prec.MAST  = TP.MAST / (TP.MAST+FP.MAST)
  Rec.MAST = TP.MAST / (TP.MAST+FN.MAST)
  
  F1.MAST = 2*(Prec.MAST*Rec.MAST/(Prec.MAST+Rec.MAST))
  
  #XAI ratio
  GO.GSEA_xai <- read.xlsx(paste0(path_GSEA,"GSEA_",celltype,"_HFpEF_t_xai.xlsx"), rowNames=T)
  GO.GSEA_xai <- GO.GSEA_xai[GO.GSEA_xai$p.adjust <= 0.05,] #Only significant
  
  nr.inGO.xai = length(GO.GSEA_xai$ID[GO.GSEA_xai$ID %in% go_id])
  nr.total.xai = length(GO.GSEA_xai$ID)
  ratio.xai = nr.inGO.xai/nr.total.xai
  
  #Precision and Recall XAI
  TP.XAI = nr.inGO.xai
  FP.XAI = nr.total.xai - TP.XAI
  FN.XAI = length(go_id) - TP.XAI
  
  Prec.XAI  = TP.XAI / (TP.XAI+FP.XAI)
  Rec.XAI = TP.XAI / (TP.XAI+FN.XAI)
  
  F1.XAI = 2*(Prec.XAI*Rec.XAI/(Prec.XAI+Rec.XAI))
  
  #rbind results to the dataframe
  df.result <- rbind(df.result, data.frame(Celltype = celltype,
                                           Seurat_Prec = Prec.Seu,
                                           Seurat_Rec = Rec.Seu,
                                           Seurat_F1 = F1.Seu,
                                           MAST_Prec = Prec.MAST,
                                           MAST_Rec = Rec.MAST,
...
#Select which things to show: F1 or Recall and Precision
df.result %<>% dplyr::select(-Seurat_Prec,-Seurat_Rec, -XAI_Prec,-XAI_Rec, -MAST_Prec, -MAST_Rec)
...
df.result.melt$variable <- plyr::revalue(df.result.melt$variable,
                                  c("Seurat_F1" = "Expr-Wilcoxon",
                                    "MAST_F1" = "Expr-MAST",
                                    "XAI_F1" = "Shap-t"))
df.result.melt$Celltype <- plyr::revalue(df.result.melt$Celltype,
                                  c("Immune.cells" = "Immune cells",
                                    "Smooth.Muscle" = "Smooth muscle"))

df.result.melt$variable <- factor(df.result.melt$variable, levels = c("Shap-t","Expr-MAST","Expr-Wilcoxon"))
...
ggplot(df.result.melt, aes(x=Celltype, y=value, shape=variable, color=variable))+
  geom_point(aes(shape=variable, size=variable), position = position_dodge(width = 0.35))+
  theme_box() +
  facet_grid(~Celltype, scales = "free")+
  scale_color_manual(name="Value type",
                     values =  dot_colours)+
  ...
  theme(axis.text.x = element_blank(),
        # axis.text.x = element_text(color = "black", size = 14,angle = 45,hjust = 1, family = "Helvetica"),
        axis.text.y = element_text(color = "black", size = 14, family = "Helvetica"),
        axis.title.x = element_blank(),
        axis.title = element_text(size = 14, color = "black", family = "Helvetica"),
...
ggsave(paste0("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/scatterplots/Figure4/Human_GSEA_F1_score_",disease,"_vs_healthy_with_MAST.pdf"), width = 12, height = 2.75)
dev.off()

```

##12.4 For cell types using(fc + p_values)
...
filtered_go_Fibroblasts <- unique(filtered_go_Fibroblasts$go_id)
#PC
filtered_go_Pericytes <- go_terms[go_terms$Term %in% go_terms$Term[grepl("Pericyte|Blood vessel morphogenesis|heart|cardiac|Regulation of vascular permeability|Vascular smooth muscle cell development|actin filament|Basement membrane organization|mesenchymal|extracellular matrix",go_terms$Term, ignore.case = TRUE)],]
filtered_go_Pericytes <- filtered_go_Pericytes[filtered_go_Pericytes$Ontology == "BP",]
filtered_go_Pericytes <- unique(filtered_go_Pericytes$go_id)
length(filtered_go_Pericytes)
#IC
filtered_go_Immune.cells <- go_terms[go_terms$Term %in% go_terms$Term[grepl("Immune response|heart|Inflammation|Leukocyte activation|Cytokine production|Antigen presentation|T cell activation|B cell differentiation|Immune cell migration|Macrophage activation|Immune system development|Interleukin signaling",go_terms$Term, ignore.case = TRUE)],]
filtered_go_Immune.cells <- filtered_go_Immune.cells[filtered_go_Immune.cells$Ontology == "BP",]
filtered_go_Immune.cells <- unique(filtered_go_Immune.cells$go_id)
...
for (celltype in cell_names) {
  ...
  #Seurat ratio
  GO.GSEA_seu <- read.xlsx(paste0(path_GSEA,"log2fc_div_p/GSEA_",celltype,"_HFpEF_wilcox_seurat_fc_p.xlsx"), rowNames=T)
  GO.GSEA_seu <- GO.GSEA_seu[GO.GSEA_seu$p.adjust <= 0.05,] #Only significant
  ...
  #MAST ratio
  GO.GSEA_MAST <- read.xlsx(paste0(path_GSEA,"log2fc_div_p/GSEA_",celltype,"_HFpEF_MAST_seurat_fc_p.xlsx"), rowNames=T)
  GO.GSEA_MAST <- GO.GSEA_MAST[GO.GSEA_MAST$p.adjust <= 0.05,] #Only significant
  ...
  #XAI ratio
  GO.GSEA_xai <- read.xlsx(paste0(path_GSEA,"log2fc/GSEA_",celltype,"_HFpEF_t_XAI.xlsx"), rowNames=T)
  GO.GSEA_xai <- GO.GSEA_xai[GO.GSEA_xai$p.adjust <= 0.05,] #Only significant
...
#Select which things to show: F1 or Recall and Precision
df.result_F1 <- df.result %>% dplyr::select(-Seurat_Prec,-Seurat_Rec, -XAI_Prec,-XAI_Rec, -MAST_Prec, -MAST_Rec)
df.result_prec <- df.result %>% dplyr::select(-Seurat_F1,-Seurat_Rec, -XAI_F1,-XAI_Rec, -MAST_F1, -MAST_Rec)

df.result.F1.melt <- reshape2::melt(df.result_F1)
df.result_prec.melt <- reshape2::melt(df.result_prec)
...
df.result.F1.melt$variable <- plyr::revalue(df.result.F1.melt$variable,
                                  c("Seurat_F1" = "Expr-Wilcoxon",
                                    "MAST_F1" = "Expr-MAST",
                                    "XAI_F1" = "Shap-t"))
df.result.F1.melt$Celltype <- plyr::revalue(df.result.F1.melt$Celltype,
                                  c("Immune.cells" = "Immune cells",
                                    "Smooth.Muscle" = "Smooth muscle"))

df.result.F1.melt$variable <- factor(df.result.F1.melt$variable, levels = c("Shap-t","Expr-MAST","Expr-Wilcoxon"))
...
ggplot(df.result.F1.melt, aes(x=Celltype, y=value, shape=variable, color=variable))+
...
df.result_prec.melt$variable <- plyr::revalue(df.result_prec.melt$variable,
                                  c("Seurat_Prec" = "Expr-Wilcoxon",
                                    "MAST_Prec" = "Expr-MAST",
                                    "XAI_Prec" = "Shap-t"))
df.result_prec.melt$Celltype <- plyr::revalue(df.result_prec.melt$Celltype,
                                  c("Immune.cells" = "Immune cells",
                                    "Smooth.Muscle" = "Smooth muscle"))

df.result_prec.melt$variable <- factor(df.result_prec.melt$variable, levels = c("Shap-t","Expr-MAST","Expr-Wilcoxon"))
...
ggplot(df.result_prec.melt, aes(x=Celltype, y=value, shape=variable, color=variable))+
  geom_point(aes(shape=variable, size=variable))+
  theme_box() +
  facet_grid(~Celltype, scales = "free")+
  scale_color_manual(name="Value type",
  ...
  scale_size_manual(name="Value type",
                    values = c(4,4,4))+
  labs(title = paste0("GSEA - Comparison of Human ", disease," vs healthy"), y = "Precision - Score", hjust=0.5) +
  theme(axis.text.x = element_blank(),
        # axis.text.x = element_text(color = "black", size = 14,angle = 45,hjust = 1, family = "Helvetica"),
        axis.text.y = element_text(color = "black", size = 14, family = "Helvetica"),
        axis.title.x = element_blank(),
...
ggsave(paste0("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/scatterplots/Figure4/Human_GSEA_prec_score_",disease,"_vs_healthy_with_MAST.pdf"), width = 12, height = 2.75)
dev.off()

```

#13. GO analysis DXG versus DEG
```{r}

#initalize cell type and disease, set testmethod to load
test.Seurat <- c("wilcox_Test", "MAST")
disease <- "HFpEF"
cell_names <- c("Cardiomyocytes","Endothelial","Fibroblasts","Immune.cells","Neuro","Pericytes","Smooth.Muscle")
...
for (method in test.Seurat) {
  ...
  for (cell_type in cell_names) {
    ...
    seurat_DGE_sub <- seurat_DGE[seurat_DGE$disease == disease & seurat_DGE$celltype == cell_type & seurat_DGE$testmethod == method.test, ]
    
    #Apply default thresholds for DEGs
    seurat_DGE_sub <- seurat_DGE_sub[seurat_DGE_sub$p_val_adj < 0.05 & abs(seurat_DGE_sub$avg_log2FC) > 0.1 & (seurat_DGE_sub$pct.1 > 0.1 | seurat_DGE_sub$pct.2 > 0.1),]
    ...
    # differentially regulated genes with ENTREZIDs as data frame
    seurat_DGE_sub_up <- seurat_DGE_sub[seurat_DGE_sub$avg_log2FC > 0,]
    seurat_DGE_sub_down <- seurat_DGE_sub[seurat_DGE_sub$avg_log2FC < 0,]
    
    #comb 
    egdiffgenes_comb <- as.vector(seurat_DGE_sub$rowname)
    egdiffgenes_comb = clusterProfiler::bitr(egdiffgenes_comb, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = OrgDb)
    
    #up genes
    egdiffgenes_up <- as.vector(seurat_DGE_sub_up$rowname)
    egdiffgenes_up = clusterProfiler::bitr(egdiffgenes_up, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = OrgDb)
    
    
    #down genes
    egdiffgenes_down <- as.vector(seurat_DGE_sub_down$rowname)
    egdiffgenes_down = clusterProfiler::bitr(egdiffgenes_down, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = OrgDb)
    
    #up go
    ego_up <- enrichGO(gene = egdiffgenes_up[,2],
                    OrgDb = OrgDb,
                    keyType = "ENTREZID",
                    ont = "BP",
    ...
    ego_up_save <- setReadable(ego_up, OrgDb = OrgDb, keyType = "ENTREZID")
    write.xlsx(ego_up_save@result,paste0("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/excelsheets/GOs/up/UP_genes_GO_",cell_type,"_",disease,"_",method,"_seurat.xlsx"), rowNames=T)
    
    #down go
    ego_down <- enrichGO(gene = egdiffgenes_down[,2],
                    OrgDb = OrgDb,
                    keyType = "ENTREZID",
                    ont = "BP",
                    universe = egallgenes[,2],
    ...
    ego_down_save <- setReadable(ego_down, OrgDb = OrgDb, keyType = "ENTREZID")
    write.xlsx(ego_down_save@result,paste0("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/excelsheets/GOs/down/DOWN_genes_GO_",cell_type,"_",disease,"_",method,"_seurat.xlsx"), rowNames=T)
    
    #down go
    ego_comb <- enrichGO(gene = egdiffgenes_comb[,2],
                    OrgDb = OrgDb,
                    keyType = "ENTREZID",
                    ont = "BP",
                    universe = egallgenes[,2],
    ...
    ego_comb_save <- setReadable(ego_comb, OrgDb = OrgDb, keyType = "ENTREZID")
    write.xlsx(ego_comb_save@result,paste0("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/excelsheets/GOs/comb/COMB_genes_GO_",cell_type,"_",disease,"_",method,"_seurat.xlsx"), rowNames=T)
    
  }
}
#DXG
for (cell_type in cell_names) {
  #set class corresponding to disease
  class_xai <- switch(disease,
                      "Hypertrophy" = "class_9",
                      "HFpEF" = "class_10",
                      "HFrEF" = "class_11",
  ...
  #Xai_0.1_0.5_0.1
  xai_path <- paste0("/media/Storage/anndata_shap/Z_SHAP/background_200_balanced_data/DGE_XAI_analysis_SHAP_tresh_0.1_log_0.5_avgExp_0.1_p_0.05/Human_DGE_XAI_analysis_SHAP_tresh_0.1_log_0.5_avgExp_0.1_p_0.05/t_test_on_",class_xai,"_Human_",celltype_xai,"_",disease_xai,"_Human_",celltype_xai,"_CTRL.xlsx")
  ...
  # differentially regulated genes with ENTREZIDs as data frame
  xai_DXG_sub_up <- xai_DGE[xai_DGE$avg_cube_root_FC > 0,]
  xai_DXG_sub_down <- xai_DGE[xai_DGE$avg_cube_root_FC < 0,]

  #comb genes
  egdiffgenes_comb <- as.vector(xai_DGE$Feature)
  egdiffgenes_comb = clusterProfiler::bitr(egdiffgenes_comb, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = OrgDb)
  
  #up genes
  egdiffgenes_up <- as.vector(xai_DXG_sub_up$Feature)
  egdiffgenes_up = clusterProfiler::bitr(egdiffgenes_up, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = OrgDb)

  #down genes
  egdiffgenes_down <- as.vector(xai_DXG_sub_down$Feature)
  egdiffgenes_down = clusterProfiler::bitr(egdiffgenes_down, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = OrgDb)
  ...
  ego_up_save <- setReadable(ego_up, OrgDb = OrgDb, keyType = "ENTREZID")
  write.xlsx(ego_up_save@result,paste0("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/excelsheets/GOs/up/UP_genes_GO_",cell_type,"_",disease,"_XAI_DXG.xlsx"), rowNames=T)
  ...
  ego_down_save <- setReadable(ego_down, OrgDb = OrgDb, keyType = "ENTREZID")
  write.xlsx(ego_down_save@result,paste0("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/excelsheets/GOs/down/DOWN_genes_GO_",cell_type,"_",disease,"_XAI_DXG.xlsx"), rowNames=T)
  ...
  ego_comb_save <- setReadable(ego_comb, OrgDb = OrgDb, keyType = "ENTREZID")
  write.xlsx(ego_comb_save@result,paste0("/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/excelsheets/GOs/comb/COMB_genes_GO_",cell_type,"_",disease,"_XAI_DXG.xlsx"), rowNames=T)
  
}
```

##13.1 GO score calculation in HFpEF (similar to 12. but with GO results)
```{r}
library(GO.db)
go_terms <- as.data.frame(GOTERM) # load all existing terms
#CM
filtered_go_Cardiomyocytes <- go_terms[go_terms$Term %in% go_terms$Term[grepl("heart|cardiac|muscle|myocardium|contraction|sarcomere|cardiogenesis|hypertrophy|electrophysiology|ion transport|gap junction|mitochondria|angiogenesis|apoptosis|fibrosis|calcium ion",go_terms$Term, ignore.case = TRUE)],]
filtered_go_Cardiomyocytes <- filtered_go_Cardiomyocytes[filtered_go_Cardiomyocytes$Ontology == "BP",]
filtered_go_Cardiomyocytes <- unique(filtered_go_Cardiomyocytes$go_id)
...
filtered_go_Fibroblasts <- unique(filtered_go_Fibroblasts$go_id)
#PC
filtered_go_Pericytes <- go_terms[go_terms$Term %in% go_terms$Term[grepl("Pericyte|Blood vessel morphogenesis|heart|Regulation of vascular permeability|Angiogenesis|Vascular smooth muscle cell development|Basement membrane organization|mesenchymal|extracellular matrix",go_terms$Term, ignore.case = TRUE)],]
filtered_go_Pericytes <- filtered_go_Pericytes[filtered_go_Pericytes$Ontology == "BP",]
filtered_go_Pericytes <- unique(filtered_go_Pericytes$go_id)
#IC
filtered_go_Immune.cells <- go_terms[go_terms$Term %in% go_terms$Term[grepl("Immune response|heart|Inflammation|Leukocyte activation|Cytokine production|Antigen presentation|T cell activation|B cell differentiation|Immune cell migration|Macrophage activation|Immune system development|Interleukin signaling",go_terms$Term, ignore.case = TRUE)],]
...
cell_names <- c("Cardiomyocytes","Endothelial","Fibroblasts","Immune.cells","Neuro","Pericytes","Smooth.Muscle")
path_GO <- "/media/Helios_scStorage/Mariano/NN_Human_Mice/Paper_R_code/excelsheets/GOs/down/"
...
for (celltype in cell_names) {
  ...
  #Seurat ratio
  GO.term_seu <- read.xlsx(paste0(path_GO,"DOWN_genes_GO_",celltype,"_HFpEF_wilcox_Test_seurat.xlsx"), rowNames=T)
  GO.term_seu <- GO.term_seu[GO.term_seu$p.adjust <= 0.05,] #Only significant
  
  nr.inGO.seu = length(GO.term_seu$ID[GO.term_seu$ID %in% go_id])
  nr.total.seu = length(GO.term_seu$ID)
  ratio.seu = nr.inGO.seu/nr.total.seu
  ...
  #MAST ratio
  GO.term_MAST <- read.xlsx(paste0(path_GO,"DOWN_genes_GO_",celltype,"_HFpEF_MAST_seurat.xlsx"), rowNames=T)
  GO.term_MAST <- GO.term_MAST[GO.term_MAST$p.adjust <= 0.05,] #Only significant
  
  nr.inGO.MAST = length(GO.term_MAST$ID[GO.term_MAST$ID %in% go_id])
  nr.total.MAST = length(GO.term_MAST$ID)
  ratio.MAST = nr.inGO.MAST/nr.total.MAST
  ...
  #XAI ratio
  GO.term_xai <- read.xlsx(paste0(path_GO,"DOWN_genes_GO_",celltype,"_HFpEF_XAI_DXG.xlsx"), rowNames=T)
  GO.term_xai <- GO.term_xai[GO.term_xai$p.adjust <= 0.05,] #Only significant
  
  nr.inGO.xai = length(GO.term_xai$ID[GO.term_xai$ID %in% go_id])
  nr.total.xai = length(GO.term_xai$ID)
  ratio.xai = nr.inGO.xai/nr.total.xai
...
ggplot(df.result.melt, aes(x=Celltype, y=value, shape=variable, color=variable))+
  ...
  scale_size_manual(name="Value type",
                    values = c(4,4,4))+
  labs(title = paste0("GO - Comparison of Human ", disease," vs healthy"), y = "F1 - Score", hjust=0.5) +
  theme(axis.text.x = element_blank(),
        # axis.text.x = element_text(color = "black", size = 14,angle = 45,hjust = 1, family = "Helvetica"),
        axis.text.y = element_text(color = "black", size = 14, family = "Helvetica"),
        axis.title.x = element_blank(),
